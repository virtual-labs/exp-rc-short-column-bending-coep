	   ...
	   +'</div>'
	    ...
	    +'<div class="row">'
	     ...
	     +'</div>'	
	   	...
	   	+'<div class="row">'
	     +'<div class="col-sm-5">'
	     +'<label class="labelstyle marginBottom">Essentricity ex (mm): </label>'
	     +'</div>'
		 +'<div class="col-sm-6">'
	     +'<input  id="ex_val" style= 10px;width:100%;"  class=" form-control marginBottom" disabled>'
	     +'</div>'	     
	     +'</div>'	
	     
	     +'<div class="row">'
	     +'<div class="col-sm-5">'
	     +'<label class="labelstyle marginBottom">Applied bending moment mx (mm) : </label>'
	     +'</div>'
		 +'<div class="col-sm-6">'
	     +'<input  id="mx_val" style= 10px;width:100%;"  class=" form-control marginBottom" disabled>'
	     +'</div>'	     
	     +'</div>'	  	     
    	   +'<div class="row">'
	   +'<div class="col-sm-5">'
...
//	     +'</div>'
	...
	$("#subPre").click(function(){
		...
		if(pre == ""){
			toastr.warning("Select Approprite value ");	
		}else{
			if(pre > 0){
		...
		toastr.warning("Select number greater than zero ");	
			}
		}
	});
	...
	$("#concreteGrade").change(function(){
		conVal = $("#concreteGrade").val();
          if(conVal==0){
	toastr.warning("Select Appropraite Values");
    }else{
	$("#subPre").prop("disabled",true);	
	$("#presText").prop("disabled",true);	
...
//		  $("#checkConfg").prop("disabled",false);
	...
	$("#lx_val").change(function(){
		lx = $("#lx_val").val();
		if(lx == 0){
			toastr.warning('submit number greater than zero');
		}else{
		  $("#steelGrade").prop("disabled",true);
		  $("#steelLabel").prop("hidden",false);	
...
//		  $("#ly_val").prop("disabled",false);
	...
	$("#ex_val").change(function(){
		ex = $("#ex_val").val();
		if(ex < 0){
			toastr.warning('submit number more than or equal to zero');
		}else{
		  $("#steelGrade").prop("disabled",true);
		  $("#steelLabel").prop("hidden",false);	
//		  $("#relTemp1").text("Strength : "+steelVal+"Mpa");
		   $("#lx_val").prop("disabled",true);
		   $("#ly_val").prop("disabled",true);
		   $("#ex_val").prop("disabled",true);
		   $("#mx_val").prop("disabled",false);
...
var flg = 0;
   $("#nextLevelForConfig").click(function(){
	...
	$("#canvas-div").html('');
	$("#main-div-conf").html('');
	var htm = '<img src="images/rcolumn.png" class="img-fluid" >'
